...
describe "page view collection" do
  ...
  it "classifies home, browse, search, post, newsletter and api by path shape" do
    post = PostFactory.create

    get_page("/")
    get_page("/posts")
    get_page("/news")
    get_page("/posts?query=crystal")
    get_page("/posts/#{post.slug}")
    get_page("/newsletter/confirmation_sent")
    get_json("/api/posts")

    kinds = PageViewQuery.new.to_a.map { |row| {row.path, row.path_kind} }
    kinds.should contain({"/", "home"})
    kinds.should contain({"/posts", "browse"})
    kinds.should contain({"/news", "browse"})
    kinds.should contain({"/posts", "search"})
    kinds.should contain({"/posts/#{post.slug}", "post"})
    # Newsletter routes are `other` by design: a confirmation click is a mail
    # client following a token, not a person arriving to read.
    kinds.should contain({"/newsletter/confirmation_sent", "other"})
    kinds.should contain({"/api/posts", "api"})
    kinds.size.should eq(7)
  end
  ...
  #
  # The succeeding POST here is a sign up: this app's newsletter subscribe
  # actions answer a 302 redirect rather than a 2xx, and a redirect does not
  # isolate "the method was refused" from "the status was refused".
