...
describe "Stats::Index" do
  around_each do |example|
    CrystalBits::Stats.lazy_rollup = false
    ...
    CrystalBits::SearchConsole.lazy_refresh = false
    begin
      example.run
    ensure
      CrystalBits::Stats.lazy_rollup = true
      CrystalBits::SearchConsole.lazy_refresh = true
    end
  end
  ...
  describe "the fetcher the page is supposed to drive" do
    it "runs a Search Console pass when the page is rendered" do
      ...
      with_search_console_property do
        CrystalBits::SearchConsole.lazy_refresh = true
        CrystalBits::SearchConsole.identity = -> {
          CrystalBits::SearchConsole::Identity.new("test-token", "app@example.iam.gserviceaccount.com")
        }
        CrystalBits::SearchConsole.transport = ->(request : CrystalBits::SearchConsole::Request) {
          requested << request.url
          CrystalBits::SearchConsole::Response.new(200, %({"rows": []}))
        }

        begin
          BrowserClient.exec(Stats::Index).status_code.should eq(200)
        ensure
          CrystalBits::SearchConsole.transport = nil
          CrystalBits::SearchConsole.identity = nil
          CrystalBits::SearchConsole.lazy_refresh = false
        end
      end

      requested.should_not be_empty
      requested.first.should contain(URI.encode_www_form("sc-domain:crystalbits.org"))
    end
  end
  ...
  describe "with data" do
    it "renders the chart, its table equivalent, and every section" do
      ...
      plant_grain(days_ago(2), "post", "/posts/why-crystal", "lobste.rs", "us", views: 120_i64, visitors: 80_i64)
      plant_grain(days_ago(1), "post", "/posts/why-crystal", "news.ycombinator.com", "de", views: 60_i64, visitors: 40_i64)
      plant_grain(days_ago(1), "post", "/posts/sharding-postgres", "lobste.rs", "us", views: 90_i64, visitors: 70_i64)
      ...
      # What people read: grain rows of kind post, ranked by views, linked.
      response.body.should contain("What people read")
      response.body.should contain(%(href="/posts/why-crystal"))
      response.body.should contain(%(href="/posts/sharding-postgres"))
      why_crystal_at = response.body.index(%(href="/posts/why-crystal")).not_nil!
      sharding_at = response.body.index(%(href="/posts/sharding-postgres")).not_nil!
      why_crystal_at.should be < sharding_at
      # Where they came from: referrers ranked, direct stated as a share.
      # Neither host appears in the layout's own links, so a match here can
      # only come from the referrers table.
      response.body.should contain("lobste.rs")
    ...
    it "labels the visitor column on lists as a sum of daily uniques" do
      ...
      plant_grain(days_ago(1), "post", "/posts/why-crystal", "lobste.rs", "us", views: 10_i64, visitors: 4_i64)
    ...
    it "restricts the window when the period changes" do
      ...
      plant_grain(days_ago(20), "post", "/posts/old-favourite", "lobste.rs", "us", views: 99_i64, visitors: 50_i64)
  ...
  describe "Search Console section" do
    ...
    it "renders queries with Google's label and the coverage date" do
      with_search_console_property do
        ...
        plant_query(days_ago(3), "crystal programming language", "https://crystalbits.org/posts/why-crystal",
          clicks: 42_i64, impressions: 900_i64, position: 3.4)
        plant_query(days_ago(4), "crystal programming language", "https://crystalbits.org/posts/why-crystal",
          clicks: 8_i64, impressions: 100_i64, position: 9.0)
        plant_query(days_ago(3), "crystalbits", "https://crystalbits.org/",
        ...
        response.body.should contain("crystal programming language")
    ...
    it "reports a failed fetch without rendering the stored figures as vanished" do
      with_search_console_property do
        ...
        plant_query(days_ago(6), "crystal newsletter", "https://crystalbits.org/",
        ...
        response.body.should contain("crystal newsletter")
      end
    end
  end
