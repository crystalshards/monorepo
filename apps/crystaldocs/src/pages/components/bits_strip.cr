...
# CrystalDocs::BitsFeed: a feed that answered with nothing renders nothing,
...
# established.
class Components::BitsStrip < Lucky::BaseComponent
  needs limit : Int32 = 3

  def render
    origin = CrystalDocs::BitsFeed.origin
    return unless origin

    articles = CrystalDocs::BitsFeed.current(limit)
  ...
  private def render_article(article : CrystalDocs::BitsFeed::Article, origin : String)
    li class: "bits-strip-item" do
      ...
      a article.title,
        href: CrystalDocs::BitsFeed.article_url(origin, article),
