...
module CrystalDocs
  ...
  # queue does not take a service account key: there is not one to take.
  #
  # Not reachable in development or test, and never called there: the
  # in-process queue is what runs outside production.
  module GoogleMetadata
    ...
    # The email of the service account this revision runs as. This is the
    # identity a Cloud Task is minted to act as, so the launcher can recognise
    # it.
    ...
    #
    # Deliberately not cached. The metadata server already caches and refreshes
    # underneath, and caching the string here would mean holding an expiring
    # credential in process memory for longer than it is valid, which fails as
    # a 401 on a code path that only runs when someone is waiting for a build.
