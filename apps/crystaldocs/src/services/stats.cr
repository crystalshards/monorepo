...
module CrystalDocs::Stats
  ...
  # docs.total_views is the number on every doc card and the ordering behind
  # the popular lists. It used to increment on every version render, which
  # counted every bot and kept no time dimension; the increment on render is
  # gone, and this rewrite from rolled view data is the only writer now.
  # The column stays because pages read it.
  #
  # Only name-addressed doc pages attribute exactly: /docs/<name>,
  # /docs/<name>/<version> and /docs/<name>/<version>/<type...> all carry
  # the package as the third path segment. Repository-addressed fronts
  # (/docs/_/<host>/<owner>/<repo>/...) carry no package name and are left
  # out rather than mis-attributed by a fuzzy repository_url match; their
  # views still exist in daily_stats under their own paths.
  #
  # Two statements, not one join: packages with no rolled views at all must
  # land on zero, because leaving the legacy count in place would keep a bot
  # number on the card. Both guard on the value actually changing so a
  # steady-state pass writes nothing.
  RECONCILE_VIEWS_SQL = <<-SQL
    UPDATE docs d
    SET total_views = pp.views,
        updated_at = NOW()
    FROM (
      SELECT split_part(path, '/', 3) AS package_name, SUM(views)::bigint AS views
      FROM daily_stats
      WHERE path_kind IN ('package', 'docs_version', 'docs_type')
        AND path LIKE '/docs/%'
        AND path NOT LIKE '/docs/_/%'
      GROUP BY 1
    ) pp
    WHERE d.package_name = pp.package_name
      AND d.total_views <> pp.views
    SQL

  RECONCILE_ZERO_SQL = <<-SQL
    UPDATE docs d
    SET total_views = 0,
        updated_at = NOW()
    WHERE d.total_views <> 0
      AND NOT EXISTS (
        SELECT 1
        FROM daily_stats s
        WHERE s.path_kind IN ('package', 'docs_version', 'docs_type')
          AND s.path LIKE '/docs/%'
          AND s.path NOT LIKE '/docs/_/%'
          AND split_part(s.path, '/', 3) = d.package_name
      )
    SQL
  ...
  # The pass itself: aggregate every ended-but-uncovered day, reconcile the
  # doc counters, advance the claim row and prune raw views, as one
  # transaction so a failure anywhere leaves none of it written. Runs inside
  # the runner seam; call ensure_fresh, not this.
  def self.run_pass : Nil
    ...
    AppDatabase.transaction do
      ...
      AppDatabase.exec(RECONCILE_VIEWS_SQL)
      AppDatabase.exec(RECONCILE_ZERO_SQL)
