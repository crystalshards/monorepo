...
# The Bits strip is one of two components on this site whose content comes
# from another service. CrystalBits answering with nothing recent is a real
...
private class BitsSource
  ...
  def install : BitsSource
    CrystalDocs::BitsFeed.transport = CrystalDocs::BitsFeed::Transport.new do
      @calls += 1
      if error = @raises
        raise error
      end
...
private def render_strip(limit = 3) : String
  Components::BitsStrip.new(limit: limit).render_to_string
end

describe Components::BitsStrip do
  before_each do
    CrystalDocs::BitsFeed.origin = BITS_ORIGIN
  end

  after_each do
    CrystalDocs::BitsFeed.origin = nil
    CrystalDocs::BitsFeed.transport = nil
    CrystalDocs::BitsFeed.reset!
  end
