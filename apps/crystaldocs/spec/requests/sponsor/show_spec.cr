...
describe Sponsor::Show do
  ...
  # For a documentation host, what money cannot buy is the whole credibility
  # question, so the statement is asserted rather than left to drift.
  it "states what sponsorship does not buy" do
    response = BrowserClient.exec(Sponsor::Show)

    response.body.should contain("What sponsorship does not buy")
    response.body.should contain(
      "Sponsorship does not buy influence over what CrystalDocs builds, hosts, or shows."
    )
    response.body.should contain("no sponsored placements anywhere on the site, at any tier")
