...
private alias SearchConsole = CrystalDocs::SearchConsole

private def rows_json(day : String, clicks : Int32 = 3, impressions : Int32 = 40, position : Float64 = 12.5) : String
  %({"rows":[{"keys":["#{day}","crystal docs","https://crystaldocs.org/"],"clicks":#{clicks},"impressions":#{impressions},"ctr":0.075,"position":#{position}}]})
end
...
describe CrystalDocs::SearchConsole do
  before_each do
    ...
    SearchConsole.identity = -> { SearchConsole::Identity.new(token: "spec-token", account_email: "docs-stats@example.test") }
  end
  ...
  describe "a pass the API refuses" do
    it "reports a 403 as an access problem naming the account and the property, never as empty data" do
      ...
      error.should contain("docs-stats@example.test")
      ...
      last_error.not_nil!.should contain("docs-stats@example.test")
