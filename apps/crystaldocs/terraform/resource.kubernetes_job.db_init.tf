...
resource "kubernetes_job" "crystaldocs_db_init" {
  metadata {
    name      = "crystaldocs-db-init"
      ...
      spec {
        ...
        init_container {
          ...
          args = [
            ...
            until pg_isready -h crystaldocs-postgres-rw -p 5432 -U app; do
          ...
          env {
            ...
            value_from {
              secret_key_ref {
                name = kubernetes_secret.crystaldocs_secrets.metadata[0].name
                key  = "database_url"
              }
            }
          }
  ...
  depends_on = [
    kubectl_manifest.crystaldocs_postgres,
    kubernetes_secret.crystaldocs_secrets
  ]

  lifecycle {
    replace_triggered_by = [
      kubernetes_secret.crystaldocs_secrets.id
    ]
  }
}
