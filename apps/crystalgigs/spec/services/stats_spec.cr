...
describe CrystalGigs::Stats do
  ...
  describe "a first pass over three days of raw views" do
    it "rolls each ended day with its own totals" do
      plant_page_view("/jobs/42", "job", stats_day(3), visitor: "a")
      plant_page_view("/jobs/42", "job", stats_day(3), visitor: "b")
      plant_page_view("/jobs", "browse", stats_day(2), visitor: "a")
      plant_page_view("/jobs/7", "job", stats_day(1), visitor: "a")
      plant_page_view("/jobs/7", "job", stats_day(1), visitor: "b")
      plant_page_view("/", "home", stats_day(1), visitor: "b")

      CrystalGigs::Stats.ensure_fresh.should be_true

      totals = CrystalGigs::Stats.daily_totals(stats_day(3), stats_day(1))
    ...
    it "advances covered_through to yesterday" do
      plant_page_view("/jobs/42", "job", stats_day(1))
  ...
  describe "a visitor seen twice in one day" do
    it "counts once in visitors and twice in views, per grain and for the day" do
      plant_page_view("/jobs/42", "job", stats_day(1), visitor: "a")
      plant_page_view("/jobs/42", "job", stats_day(1), visitor: "a")

      CrystalGigs::Stats.ensure_fresh

      stat_rows.call(stats_day(1)).should contain({"job", 2_i64, 1_i64})
      stat_rows.call(stats_day(1)).should contain({"site", 2_i64, 1_i64})

      total = CrystalGigs::Stats.daily_totals(stats_day(1), stats_day(1)).first
  ...
  describe "today" do
    it "is never rolled up, because it is still accumulating" do
      plant_page_view("/jobs/42", "job", Time.utc, visitor: "a")

      CrystalGigs::Stats.ensure_fresh.should be_true

      stat_rows.call(Time.utc).should be_empty
      CrystalGigs::Stats.daily_totals(Time.utc, Time.utc).first.views.should eq(0)
  ...
  describe "the retry floor" do
    it "lets a failed pass be reclaimed only after the floor" do
      calls = 0
      original = CrystalGigs::Stats.runner
      CrystalGigs::Stats.runner = -> { calls += 1; raise "boom" }
      begin
        CrystalGigs::Stats.ensure_fresh.should be_false
        CrystalGigs::Stats.ensure_fresh.should be_false
        CrystalGigs::Stats.ensure_fresh.should be_false
        calls.should eq(1)
      ensure
        CrystalGigs::Stats.runner = original
      end
      ...
      AppDatabase.exec(
        ...
        CrystalGigs::Stats::RETRY_FLOOR.ago - 1.minute
      )

      plant_page_view("/jobs/42", "job", stats_day(1))
      CrystalGigs::Stats.ensure_fresh.should be_true
      CrystalGigs::Stats.daily_totals(stats_day(1), stats_day(1)).first.views.should eq(1)
    ...
    it "never redoes a pass that already succeeded" do
      calls = 0
      original = CrystalGigs::Stats.runner
      CrystalGigs::Stats.runner = -> { calls += 1; original.call }
      begin
        plant_page_view("/jobs/42", "job", stats_day(1))
  ...
  describe "a pass that will not finish in time" do
    it "gives up waiting rather than hanging the render" do
      original_runner = CrystalGigs::Stats.runner
      original_timeout = CrystalGigs::Stats.rollup_timeout
      CrystalGigs::Stats.rollup_timeout = 50.milliseconds
      CrystalGigs::Stats.runner = -> { sleep 2.seconds; true }
      begin
        ...
        CrystalGigs::Stats.rollup_timeout = original_timeout
        CrystalGigs::Stats.runner = original_runner
      end
    end
  end
  ...
  describe "re-rolling a covered day" do
    it "overwrites the grain instead of double counting it" do
      plant_page_view("/jobs/42", "job", stats_day(2), visitor: "a")
      plant_page_view("/jobs/42", "job", stats_day(1), visitor: "a")
      CrystalGigs::Stats.ensure_fresh
      ...
      AppDatabase.exec(
        ...
        stats_day(3).to_s("%F"), CrystalGigs::Stats::RETRY_FLOOR.ago - 1.minute
      )

      CrystalGigs::Stats.ensure_fresh.should be_true

      total = CrystalGigs::Stats.daily_totals(stats_day(2), stats_day(2)).first
  ...
  describe "pruning" do
    it "removes raw rows past the window and keeps newer ones" do
      plant_page_view("/jobs/1", "job", stats_day(31), visitor: "a")
      plant_page_view("/jobs/2", "job", stats_day(29), visitor: "a")

      CrystalGigs::Stats.ensure_fresh

      AppDatabase.query_all("SELECT path FROM page_views", as: String).should eq(["/jobs/2"])
      # The pruned day was rolled before it was deleted, so its numbers
      # survive in daily_stats.
      CrystalGigs::Stats.daily_totals(stats_day(31), stats_day(31)).first.views.should eq(1)
    end
  end

  describe "the breakdowns the page renders" do
    it "ranks the most read paths of a kind, most views first" do
      3.times { |i| plant_page_view("/jobs/42", "job", stats_day(2), visitor: "v#{i}") }
      plant_page_view("/jobs/7", "job", stats_day(1), visitor: "a")
      plant_page_view("/", "home", stats_day(1), visitor: "a")

      CrystalGigs::Stats.ensure_fresh

      paths = CrystalGigs::Stats.top_paths("job", stats_day(7), stats_day(1))
      paths.map(&.path).should eq(["/jobs/42", "/jobs/7"])
      paths.first.views.should eq(3)
      paths.first.visitors.should eq(3)

      CrystalGigs::Stats.top_paths("job", stats_day(7), stats_day(1), limit: 1).size.should eq(1)
    end

    it "ranks several kinds together when the page reads them as one list" do
      plant_page_view("/jobs/42", "job", stats_day(1), visitor: "a")
      plant_page_view("/jobs", "browse", stats_day(1), visitor: "a")
      plant_page_view("/jobs", "browse", stats_day(1), visitor: "b")
      plant_page_view("/", "home", stats_day(1), visitor: "a")

      CrystalGigs::Stats.ensure_fresh

      paths = CrystalGigs::Stats.top_paths(["job", "browse"], stats_day(7), stats_day(1))
      paths.map(&.path).should eq(["/jobs", "/jobs/42"])
    end

    it "ranks referrer hosts, with direct as its own bucket" do
      plant_page_view("/jobs/42", "job", stats_day(1), visitor: "a", referrer_host: "reddit.com")
      plant_page_view("/jobs/42", "job", stats_day(1), visitor: "b", referrer_host: "reddit.com")
      plant_page_view("/jobs/7", "job", stats_day(1), visitor: "c")

      CrystalGigs::Stats.ensure_fresh

      referrers = CrystalGigs::Stats.top_referrers(stats_day(7), stats_day(1))
    ...
    it "ranks countries, with unknown for views that carried no geo header" do
      plant_page_view("/jobs/42", "job", stats_day(1), visitor: "a", country: "US")
      plant_page_view("/jobs/7", "job", stats_day(1), visitor: "b")

      CrystalGigs::Stats.ensure_fresh

      countries = CrystalGigs::Stats.top_countries(stats_day(7), stats_day(1))
  ...
  describe ".previous_period" do
    it "is the same length window immediately before" do
      previous = CrystalGigs::Stats.previous_period(stats_day(7), stats_day(4))
  ...
  describe ".covered_through" do
    it "is nil before the first pass and yesterday after it" do
      CrystalGigs::Stats.covered_through.should be_nil

      CrystalGigs::Stats.ensure_fresh

      yesterday = stats_day(1)
      CrystalGigs::Stats.covered_through.should eq(Time.utc(yesterday.year, yesterday.month, yesterday.day))
    end
  end
end
