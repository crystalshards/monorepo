...
describe "Stats::Index" do
  # The four stats tables have no model in this app, so the global cleaner
  # never names them; each example starts from a genuinely empty state.
  before_each do
  ...
  around_each do |example|
    CrystalGigs::Stats.lazy_rollup = false
    ...
    CrystalGigs::SearchConsole.lazy_refresh = false
    begin
      example.run
    ensure
      CrystalGigs::Stats.lazy_rollup = true
      CrystalGigs::SearchConsole.lazy_refresh = true
    end
  end
  ...
  describe "the fetcher the page is supposed to drive" do
    it "runs a Search Console pass when the page is rendered" do
      ...
      with_search_console_property do
        CrystalGigs::SearchConsole.lazy_refresh = true
        CrystalGigs::SearchConsole.identity = -> {
          CrystalGigs::SearchConsole::Identity.new("test-token", "app@example.iam.gserviceaccount.com")
        }
        CrystalGigs::SearchConsole.transport = ->(request : CrystalGigs::SearchConsole::Request) {
          requested << request.url
          CrystalGigs::SearchConsole::Response.new(200, %({"rows": []}))
        }

        begin
          BrowserClient.exec(Stats::Index).status_code.should eq(200)
        ensure
          CrystalGigs::SearchConsole.transport = nil
          CrystalGigs::SearchConsole.identity = nil
          CrystalGigs::SearchConsole.lazy_refresh = false
        end
      end

      requested.should_not be_empty
      requested.first.should contain(URI.encode_www_form("sc-domain:crystalgigs.com"))
    end
  end
  ...
  describe "with no data at all" do
    it "says so instead of rendering zeros" do
      ...
      response.body.should contain("Site statistics")
  ...
  describe "with data" do
    it "renders the chart, its table equivalent, and every section" do
      ...
      plant_grain(days_ago(2), "browse", "/jobs", "github.com", "us", views: 120_i64, visitors: 80_i64)
      plant_grain(days_ago(1), "browse", "/jobs", "news.ycombinator.com", "de", views: 60_i64, visitors: 40_i64)
      # A posting's own page rolls up under `job` but is never listed: the
      # board charges to post, so a public ranking of postings would publish
      # each employer's response rate, and their competitors' too.
      plant_grain(days_ago(1), "job", "/jobs/42", "github.com", "us", views: 500_i64, visitors: 300_i64)
      ...
      # What people read: the listing, linked under its real path, with its
      # two browse grains summed (120 + 60 views). The posting never appears.
      response.body.should contain(%(<a href="/jobs">/jobs</a>))
      response.body.should contain(%(<td class="num">180</td>))
      response.body.should_not contain("/jobs/42")
    ...
    it "labels the visitor column on lists as a sum of daily uniques" do
      ...
      plant_grain(days_ago(1), "browse", "/jobs", "github.com", "us", views: 10_i64, visitors: 4_i64)
    ...
    it "restricts the window when the period changes" do
      ...
      plant_grain(days_ago(20), "browse", "/jobs", "lobste.rs", "us", views: 99_i64, visitors: 50_i64)

      week = BrowserClient.exec(Stats::Index.with(period: "7"))
      chart_days(week.body).should eq(7)
      week.body.should_not contain("lobste.rs")
      week.body.should contain("Nothing read in this period")

      month = BrowserClient.exec(Stats::Index.with(period: "30"))
      chart_days(month.body).should eq(30)
      month.body.should contain("lobste.rs")
      month.body.should contain(%(<a href="/jobs">/jobs</a>))
    end
  ...
  describe "Search Console section" do
    ...
    it "renders queries with Google's label and the coverage date" do
      with_search_console_property do
        ...
        plant_query(days_ago(3), "crystal jobs", "https://crystalgigs.com/jobs",
          clicks: 42_i64, impressions: 900_i64, position: 3.4)
        plant_query(days_ago(4), "crystal jobs", "https://crystalgigs.com/jobs",
          clicks: 8_i64, impressions: 100_i64, position: 9.0)
        plant_query(days_ago(3), "remote crystal jobs", "https://crystalgigs.com/",
        ...
        response.body.should contain("remote crystal jobs")
    ...
    it "reports a failed fetch without rendering the stored figures as vanished" do
      with_search_console_property do
        ...
        plant_query(days_ago(6), "crystal contract work", "https://crystalgigs.com/",
        ...
        response.body.should contain("crystal contract work")
      end
    end
  end
