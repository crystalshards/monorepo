...
describe "page view collection" do
  ...
  it "classifies home, browse, search, job, other and api by path shape" do
    job = JobFactory.create

    get_page("/")
    get_page("/jobs")
    get_page("/jobs?query=crystal")
    get_page("/jobs/#{job.id}")
    # The posting form is deliberately `other`: it is the top of the paid
    # funnel, and funnel traffic is not an audience number.
    get_page("/jobs/new")
    get_json("/api/jobs")

    kinds = PageViewQuery.new.to_a.map { |row| {row.path, row.path_kind} }
    kinds.should contain({"/", "home"})
    kinds.should contain({"/jobs", "browse"})
    kinds.should contain({"/jobs", "search"})
    kinds.should contain({"/jobs/#{job.id}", "job"})
    kinds.should contain({"/jobs/new", "other"})
    kinds.should contain({"/api/jobs", "api"})
    kinds.size.should eq(6)
  end
