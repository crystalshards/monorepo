...
describe Jobs::Index do
  it "renders the job listing page" do
    ...
    response = AppClient.exec(Jobs::Index)
  ...
  it "filters by location" do
    ...
    response = AppClient.exec(Jobs::Index, location: "San Francisco")
  ...
  it "filters by job type" do
    JobFactory.create &.title("Full Time").job_type("full-time").published_at(Time.utc).active(true)
    JobFactory.create &.title("Contract").job_type("contract").published_at(Time.utc).active(true)

    response = AppClient.exec(Jobs::Index, job_type: "contract")

    response.status_code.should eq(200)
    response.body.should contain("Contract")
    response.body.should_not contain("Full Time")
  end
  ...
  it "filters by remote" do
    ...
    response = AppClient.exec(Jobs::Index, remote: "true")
  ...
  it "searches jobs by query" do
    ...
    JobFactory.create &.title("Ruby Developer").published_at(Time.utc).active(true)

    response = AppClient.exec(Jobs::Index, query: "Crystal")
  ...
  it "paginates results" do
    ...
    response = AppClient.exec(Jobs::Index, page: "2")
