# Schema migration entrypoint for the crystalgigs-migrate Cloud Run Job.
#
...
# config/payments.cr calls exit(1) without STRIPE_PUBLISHABLE_KEY, and apps add more
# over time. None of it is reachable from a migration, so requiring the app would
# hand the one identity holding DDL rights a session signing key and a third-party
# credential to run a DDL statement, and every new boot time config would silently
# break migrations until somebody remembered to add another variable to the Job.
#
