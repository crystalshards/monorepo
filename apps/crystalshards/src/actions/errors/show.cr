...
class Errors::Show < Lucky::ErrorAction
  ...
  def render(error : Lucky::RouteNotFoundError | Avram::RecordNotFoundError)
    if html?
      error_html "Not found", status: 404
    else
  ...
  def render(error : Avram::InvalidOperationError)
    if html?
      error_html error.renderable_message, status: 400
    else
      error_json \
  ...
  def default_render(error : Exception) : Lucky::Response
    if LuckyEnv.test?
      ...
      if html?
        error_html "#{DEFAULT_MESSAGE} #{error.message}", status: 500
      else
  ...
  private def error_html(message : String, status : Int)
    html Errors::ShowPage, message: message, status: status, _with_status_code: status
  end
