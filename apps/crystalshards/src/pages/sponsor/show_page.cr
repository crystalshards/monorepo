class Sponsor::ShowPage < MainLayout
  ...
  # The ask comes after the case for it: what the registry does, what running
  # it costs, what the money cannot buy, and who is asking. Only then the
    ...
    section class: "intro" do
      div class: "intro-copy" do
        ...
        para class: "intro-lede" do
          text "CrystalShards indexes every tagged release of every published " \
               "Crystal shard and serves the dependency data the ecosystem " \
               "builds on. The Bushido Collective maintains it and pays for " \
               "it out of pocket. Sponsorship covers the infrastructure bill."
        end
      end
    end
  end
  ...
  private def render_costs
    section class: "section" do
      ...
      ul class: "sponsor-costs" do
        cost_item "Compute",
          "Four services run on Cloud Run: this registry, the documentation " \
          "host, the job board and the writing. They are metered by the " \
          "request and by the CPU-second, so a busier ecosystem costs more " \
          "to serve."
        cost_item "The database",
          "Cloud SQL holds every shard, version, dependency link and crawl " \
          "record the registry has ever indexed. It is sized to stay fast as " \
          "that grows, not to be cheap on the day it was provisioned."
        cost_item "Storage",
          "Object storage keeps every published package tarball and one " \
          "documentation build for every version of every shard. Every tag " \
          "anyone pushes adds to it, and none of it is deleted."
        cost_item "The build sandbox",
          "Every documentation build compiles a third party shard inside a " \
          "locked down job with no network and no credentials, because a " \
          "shard's macros can execute code at compile time. That confinement " \
          "is what makes the hosted documentation safe to serve, it runs once " \
          "per release, and it is billed per run."
        cost_item "Bandwidth and the small print",
          "Serving pages and tarballs, plus DNS, logging and monitoring. " \
  ...
  # For a registry this is the whole credibility question, so it gets its own
  # section in plain words rather than a line of fine print. The spec suite
  # asserts this statement verbatim; soften it and the build goes red.
  private def render_no_influence
    section class: "section" do
      ...
      div class: "sponsor-copy" do
        para do
          text "Sponsorship does not buy influence over what this registry " \
               "indexes, ranks, or shows. Every tagged release is indexed the " \
               "same way whether its author sponsors or not, and nothing " \
               "about a shard's page, its place in search results, or its " \
               "position in any list is for sale. Ranking comes from star " \
               "counts and dependency links, which are measured from public " \
               "data, not negotiated. There are no sponsored listings and no " \
               "paid placement at any tier, and there will not be. If that " \
               "ever changes, it changes on this page first."
        end
      end
    end
  end
  ...
  private def render_maintainer
    section class: "section" do
      ...
      div class: "sponsor-copy" do
        para do
          text "CrystalShards is built and maintained by The Bushido " \
               "Collective, the same small group of working engineers behind " \
               "CrystalDocs, CrystalGigs and CrystalBits. It is not a " \
        ...
        if destination = Sponsorship.destination
          ...
          para do
            text "Sponsorship is not open yet. We are still deciding where " \
                 ...
                 "most useful thing you can do is use the registry and " \
                 "publish to it."
          end
        end
      end
