...
# CrystalShards::BitsFeed: a feed that answered with nothing renders nothing,
...
class BitsStrip < Lucky::BaseComponent
  ...
  def render
    origin = CrystalShards::BitsFeed.origin
    return unless origin

    articles = CrystalShards::BitsFeed.current(limit)
  ...
  private def render_article(article : CrystalShards::BitsFeed::Article, origin : String)
    li class: "bits-strip-item" do
      ...
      a article.title,
        href: CrystalShards::BitsFeed.article_url(origin, article),
