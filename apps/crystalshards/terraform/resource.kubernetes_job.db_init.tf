...
resource "kubernetes_job" "crystalshards_db_init" {
  metadata {
    name      = "crystalshards-db-init"
      ...
      spec {
        ...
        init_container {
          ...
          args = [
            ...
            until pg_isready -h crystalshards-postgres-rw -p 5432 -U app; do
          ...
          env {
            ...
            value_from {
              secret_key_ref {
                name = kubernetes_secret.crystalshards_secrets.metadata[0].name
                key  = "database_url"
              }
            }
          }
  ...
  depends_on = [
    kubectl_manifest.crystalshards_postgres,
    kubernetes_secret.crystalshards_secrets
  ]

  lifecycle {
    replace_triggered_by = [
      kubernetes_secret.crystalshards_secrets.id
    ]
  }
}
