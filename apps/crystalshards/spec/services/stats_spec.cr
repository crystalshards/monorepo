...
describe Stats do
  ...
  describe "a first pass over three days of raw views" do
    it "rolls each ended day with its own totals" do
      plant_page_view("/shards/kemal", "package", stats_day(3), visitor: "a")
      plant_page_view("/shards/kemal", "package", stats_day(3), visitor: "b")
      plant_page_view("/shards/kemal/versions", "other", stats_day(2), visitor: "a")
      plant_page_view("/shards/semver", "package", stats_day(1), visitor: "a")
      plant_page_view("/shards/semver", "package", stats_day(1), visitor: "b")
      plant_page_view("/search?q=semver", "search", stats_day(1), visitor: "b")

      Stats.ensure_fresh.should be_true

      totals = Stats.daily_totals(stats_day(3), stats_day(1))
    ...
    it "advances covered_through to yesterday" do
      plant_page_view("/shards/kemal", "package", stats_day(1))
  ...
  describe "a visitor seen twice in one day" do
    it "counts once in visitors and twice in views, per grain and for the day" do
      plant_page_view("/shards/kemal", "package", stats_day(1), visitor: "a")
      plant_page_view("/shards/kemal", "package", stats_day(1), visitor: "a")
      ...
      total = Stats.daily_totals(stats_day(1), stats_day(1)).first
  ...
  describe "today" do
    it "is never rolled up, because it is still accumulating" do
      plant_page_view("/shards/kemal", "package", Time.utc, visitor: "a")

      Stats.ensure_fresh.should be_true

      stat_rows.call(Time.utc).should be_empty
      Stats.daily_totals(Time.utc, Time.utc).first.views.should eq(0)
  ...
  describe "the retry floor" do
    it "lets a failed pass be reclaimed only after the floor" do
      calls = 0
      original = Stats.runner
      Stats.runner = -> { calls += 1; raise "boom" }
      begin
        Stats.ensure_fresh.should be_false
        Stats.ensure_fresh.should be_false
        Stats.ensure_fresh.should be_false
        calls.should eq(1)
      ensure
        Stats.runner = original
      end
      ...
      AppDatabase.exec(
        ...
        Stats::RETRY_FLOOR.ago - 1.minute
      )

      plant_page_view("/shards/kemal", "package", stats_day(1))
      Stats.ensure_fresh.should be_true
      Stats.daily_totals(stats_day(1), stats_day(1)).first.views.should eq(1)
    ...
    it "never redoes a pass that already succeeded" do
      calls = 0
      original = Stats.runner
      Stats.runner = -> { calls += 1; original.call }
      begin
        plant_page_view("/shards/kemal", "package", stats_day(1))
  ...
  describe "a pass that will not finish in time" do
    it "gives up waiting rather than hanging the render" do
      original_runner = Stats.runner
      original_timeout = Stats.rollup_timeout
      Stats.rollup_timeout = 50.milliseconds
      Stats.runner = -> { sleep 2.seconds; true }
      begin
        ...
        Stats.rollup_timeout = original_timeout
        Stats.runner = original_runner
      end
    end
  end
  ...
  describe "re-rolling a covered day" do
    it "overwrites the grain instead of double counting it" do
      plant_page_view("/shards/kemal", "package", stats_day(2), visitor: "a")
      plant_page_view("/shards/kemal", "package", stats_day(1), visitor: "a")
      Stats.ensure_fresh
      ...
      AppDatabase.exec(
        ...
        stats_day(3).to_s("%F"), Stats::RETRY_FLOOR.ago - 1.minute
      )

      Stats.ensure_fresh.should be_true

      total = Stats.daily_totals(stats_day(2), stats_day(2)).first
  ...
  describe "pruning" do
    it "removes raw rows past the window and keeps newer ones" do
      plant_page_view("/shards/old", "package", stats_day(31), visitor: "a")
      plant_page_view("/shards/recent", "package", stats_day(29), visitor: "a")

      Stats.ensure_fresh

      AppDatabase.query_all("SELECT path FROM page_views", as: String).should eq(["/shards/recent"])
      # The pruned day was rolled before it was deleted, so its numbers
      # survive in daily_stats.
      Stats.daily_totals(stats_day(31), stats_day(31)).first.views.should eq(1)
    end
  end

  describe "the breakdowns the page renders" do
    it "ranks the most read paths of a kind, most views first" do
      3.times { |i| plant_page_view("/shards/kemal", "package", stats_day(2), visitor: "v#{i}") }
      plant_page_view("/shards/semver", "package", stats_day(1), visitor: "a")
      plant_page_view("/search?q=kemal", "search", stats_day(1), visitor: "a")

      Stats.ensure_fresh

      paths = Stats.top_paths("package", stats_day(7), stats_day(1))
      paths.map(&.path).should eq(["/shards/kemal", "/shards/semver"])
      paths.first.views.should eq(3)
      paths.first.visitors.should eq(3)

      Stats.top_paths("package", stats_day(7), stats_day(1), limit: 1).size.should eq(1)
    end

    it "ranks several kinds together when the page reads them as one list" do
      plant_page_view("/shards/kemal", "package", stats_day(1), visitor: "a")
      plant_page_view("/search?q=kemal", "search", stats_day(1), visitor: "a")
      plant_page_view("/search?q=kemal", "search", stats_day(1), visitor: "b")
      plant_page_view("/", "home", stats_day(1), visitor: "a")

      Stats.ensure_fresh

      paths = Stats.top_paths(["package", "search"], stats_day(7), stats_day(1))
      paths.map(&.path).should eq(["/search?q=kemal", "/shards/kemal"])
    end

    it "ranks referrer hosts, with direct as its own bucket" do
      plant_page_view("/shards/kemal", "package", stats_day(1), visitor: "a", referrer_host: "reddit.com")
      plant_page_view("/shards/kemal", "package", stats_day(1), visitor: "b", referrer_host: "reddit.com")
      plant_page_view("/shards/semver", "package", stats_day(1), visitor: "c")

      Stats.ensure_fresh

      referrers = Stats.top_referrers(stats_day(7), stats_day(1))
    ...
    it "ranks countries, with unknown for views that carried no geo header" do
      plant_page_view("/shards/kemal", "package", stats_day(1), visitor: "a", country: "US")
      plant_page_view("/shards/semver", "package", stats_day(1), visitor: "b")

      Stats.ensure_fresh

      countries = Stats.top_countries(stats_day(7), stats_day(1))
  ...
  describe ".previous_period" do
    it "is the same length window immediately before" do
      previous = Stats.previous_period(stats_day(7), stats_day(4))
  ...
  describe ".covered_through" do
    it "is nil before the first pass and yesterday after it" do
      Stats.covered_through.should be_nil

      Stats.ensure_fresh

      yesterday = stats_day(1)
      Stats.covered_through.should eq(Time.utc(yesterday.year, yesterday.month, yesterday.day))
    end
  end
end
