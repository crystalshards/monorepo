...
# `CodeHighlighter` is the module `CrystalShards::ReadmeHtml` runs a README
# fence through; its twin at `CrystalDocs::DocHtml` does the same for that
# app. These specs exercise this copy directly, one language at a time,
# rather than through the renderer: what a key, a value, a comment or a
# fallback becomes is a property of this module, not of Markdown or of the
# sanitiser layered on top of it.
describe CrystalShards::CodeHighlighter do
  describe "Crystal" do
    it "delegates to the compiler's own highlighter" do
      html = CrystalShards::CodeHighlighter.highlight(%(def greet\n  "hi" # wave\nend), "crystal")
    ...
    it "accepts the short alias" do
      CrystalShards::CodeHighlighter.highlight("1 + 1", "cr")
        .should contain(%(<span class="n">1</span>))
    end

    it "falls back to plain escaped source when the lexer rejects the block" do
      html = CrystalShards::CodeHighlighter.highlight("$ shards install", "crystal")
  ...
  describe "YAML" do
    ...
    it "colours the boolean and null spellings YAML accepts as keywords" do
      {"true", "false", "yes", "no", "on", "off", "null", "~"}.each do |literal|
        CrystalShards::CodeHighlighter.highlight("flag: #{literal}", "yaml")
          .should contain(%(<span class="k">#{literal}</span>))
      end
    end

    it "colours a full-line comment" do
      CrystalShards::CodeHighlighter.highlight("# top level note", "yaml")
    ...
    it "colours a trailing comment after a value, keeping the gap between them plain" do
      html = CrystalShards::CodeHighlighter.highlight("port: 8080 # the default", "yaml")
    ...
    it "colours an anchor and an alias as a named reference" do
      html = CrystalShards::CodeHighlighter.highlight("a: &base\nb: *base", "yaml")
    ...
    it "colours an anchored inline value on both halves" do
      html = CrystalShards::CodeHighlighter.highlight("key: &name value", "yaml")
    ...
    it "colours the document start and end markers" do
      html = CrystalShards::CodeHighlighter.highlight("---\nname: kemal\n...", "yaml")
    ...
    it "keeps a sequence dash plain and colours the item after it" do
      html = CrystalShards::CodeHighlighter.highlight("items:\n  - one\n  - 2", "yaml")
    ...
    it "leaves a flow collection uncoloured rather than guessing what is inside it" do
      html = CrystalShards::CodeHighlighter.highlight("list: [a, b, c]", "yaml")

      html.should eq(%(<span class="m">list:</span> [a, b, c]))
    end

    it "leaves a block scalar header uncoloured" do
      CrystalShards::CodeHighlighter.highlight("body: |", "yaml")
    ...
    it "escapes a value exactly once" do
      html = CrystalShards::CodeHighlighter.highlight("name: Q&A", "yaml")
  ...
  describe "shell" do
    it "colours the command name at the head of a line" do
      CrystalShards::CodeHighlighter.highlight("shards install", "shell")
        .should eq(%(<span class="m">shards</span> install))
    end

    it "colours a flag but not a plain argument" do
      html = CrystalShards::CodeHighlighter.highlight("brew install --cask crystal", "bash")
    ...
    it "colours the command name again after a pipe" do
      html = CrystalShards::CodeHighlighter.highlight("curl -fsSL https://example.com | sh", "zsh")
  ...
  describe "shell session" do
    it "colours the prompt and the command that follows it" do
      html = CrystalShards::CodeHighlighter.highlight("$ shards install", "console")
    ...
    it "does not mistake a variable expansion for a prompt in a plain script" do
      CrystalShards::CodeHighlighter.highlight("echo $HOME", "bash")
  ...
  describe "JSON" do
    it "gives a key and a string value different classes" do
      html = CrystalShards::CodeHighlighter.highlight(%({"name": "kemal"}), "json")
    ...
    it "colours a number, including a negative exponent" do
      CrystalShards::CodeHighlighter.highlight(%({"n": 3000}), "json")
        .should contain(%(<span class="n">3000</span>))
      CrystalShards::CodeHighlighter.highlight(%({"n": -1.5e10}), "json")
        .should contain(%(<span class="n">-1.5e10</span>))
    end

    it "colours the three literals as keywords" do
      html = CrystalShards::CodeHighlighter.highlight(%({"a": true, "b": false, "c": null}), "json")
    ...
    it "leaves structural punctuation uncoloured" do
      html = CrystalShards::CodeHighlighter.highlight(%({"a": 1, "b": 2}), "json")
    ...
    it "escapes a value exactly once" do
      html = CrystalShards::CodeHighlighter.highlight(%({"note": "Q&A"}), "json")
  ...
  describe "an unsupported or absent language" do
    it "escapes the source and never colours it" do
      html = CrystalShards::CodeHighlighter.highlight(%(name = "kemal"), "toml")
    ...
    it "does the same for an unlabelled fence" do
      html = CrystalShards::CodeHighlighter.highlight("plain <text> & stuff", nil)
    ...
    it "matches language names case-insensitively" do
      CrystalShards::CodeHighlighter.highlight("port: 80", "YAML")
  ...
  describe "empty input" do
    it "returns an empty string for every language rather than raising" do
      {"crystal", "yaml", "json", "shell", "console", nil, "toml"}.each do |language|
        CrystalShards::CodeHighlighter.highlight("", language).should eq("")
      end
    end
  end
end
