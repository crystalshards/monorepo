class CreateSearchConsoleDaily::V00000000000024 < Avram::Migrator::Migration::V1
  def migrate
    ...
    # by CrystalShards::SearchConsole's upsert and read only by the stats
    # page, both raw SQL, so a model would be a third copy of the schema to
    # keep in agreement rather than a convenience anyone uses.
    execute <<-SQL
