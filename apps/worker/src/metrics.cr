...
module Metrics
  ...
  # Application-specific metrics for CrystalWorker
  ...
  JOBS_PROCESSED_TOTAL = REGISTRY.counter("jobs_processed_total", "Total background jobs processed")
  JOB_PROCESSING_DURATION = REGISTRY.histogram("job_processing_duration_seconds", "Job processing duration in seconds")
  JOB_FAILURES_TOTAL = REGISTRY.counter("job_failures_total", "Total job processing failures")
  DATABASE_CONNECTIONS = REGISTRY.gauge("database_connections", "Current database connections")
  QUEUE_SIZE = REGISTRY.gauge("queue_size", "Current queue size")
  ACTIVE_WORKERS = REGISTRY.gauge("active_workers", "Number of active worker processes")
